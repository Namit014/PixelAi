-- Keep pricing plan configuration current and idempotent
...
-- Fresh accounts must start with zero credits and no active plan
...
BEGIN
  ...
  ON CONFLICT (user_id) DO UPDATE
     SET balance = public.credits.balance + COALESCE(plan.credits_monthly, 0),
...
DECLARE
  ...
  tbls text[] := ARRAY[
    ...
    'design_generations','feedback','support_tickets','discount_codes','invite_codes',
