...
// Configuration
const SITE_NAME = "colabcanvas"
const SENDER_DOMAIN = "notify.letscolab.in"
const FROM_DOMAIN = "letscolab.in"
const APP_DOMAIN = "letscolab.in"

const SAMPLE_PROJECT_URL = "https://colabcanvas.lovable.app"
...
async function fetchEmailSettings(): Promise<Record<string, any> | null> {
  try {
...
// Map DB settings to template props by email type
function getSettingsProps(settings: Record<string, any> | null, emailType: string): Record<string, string> {
  if (!settings) return {};
  const base: Record<string, string> = {
    logoUrl: settings.logo_url || '',
    brandName: settings.brand_name || '',
    primaryColor: settings.primary_color || '#18181b',
    buttonRadius: settings.button_radius || '12px',
    footerText: settings.footer_text || '',
    supportEmail: settings.support_email || '',
  };
  switch (emailType) {
    case 'signup':
      return { ...base, headingOverride: settings.signup_heading, bodyOverride: settings.signup_body, buttonOverride: settings.signup_button };
    case 'recovery':
      return { ...base, headingOverride: settings.recovery_heading, bodyOverride: settings.recovery_body, buttonOverride: settings.recovery_button };
    case 'magiclink':
      return { ...base, headingOverride: settings.magiclink_heading, bodyOverride: settings.magiclink_body, buttonOverride: settings.magiclink_button };
    default:
      return base;
  }
}
...
async function handlePreview(req: Request): Promise<Response> {
  ...
  const settingsProps = getSettingsProps(settings, type);
  const sampleData = { ...(SAMPLE_DATA[type] || {}), ...settingsProps };
...
async function handleWebhook(req: Request): Promise<Response> {
  ...
  // Fetch customization settings from DB
  const settings = await fetchEmailSettings();
  const settingsProps = getSettingsProps(settings, emailType);
  ...
  const fromName = settings?.brand_name || SITE_NAME;
