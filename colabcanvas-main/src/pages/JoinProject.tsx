...
import { Loader2, Users, Eye, Edit3, AlertCircle, CheckCircle } from 'lucide-react';
...
export default function JoinProject() {
  ...
  useEffect(() => {
    if (!authLoading && shareToken) {
      if (!user) {
        ...
        navigate(`/auth?redirect=/project/join/${shareToken}`);
        return;
      }
      validateShare();
    }
  ...
  const validateShare = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke('accept-project-share', {
      ...
      if (data.isOwner) {
        ...
        navigate(`/canvas?projectId=${data.projectId}`);
        return;
      }
        ...
        <CardContent className="space-y-6">
          {error ? (
            ...
            <div className="text-center space-y-4">
              ...
              <p className="text-muted-foreground text-sm">Redirecting to canvas...</p>
            </div>
          ) : shareInfo ? (
            <>
              <div className="bg-muted/50 rounded-lg p-4 space-y-3">
                <div>
                  ...
                  <p className="font-semibold text-lg">{shareInfo.projectTitle}</p>
                </div>
              ...
              <p className="text-xs text-muted-foreground text-center">
                By joining, you'll be able to {shareInfo.permission === 'edit' ? 'view and edit' : 'view'} this project.
              </p>
            </>
          ) : null}
        </CardContent>
