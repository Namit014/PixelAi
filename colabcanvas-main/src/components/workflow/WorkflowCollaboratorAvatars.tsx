...
interface WorkflowCollaboratorAvatarsProps {
  workflowId: string;
  currentUserId: string;
  maxVisible?: number;
}

export function WorkflowCollaboratorAvatars({ workflowId, currentUserId, maxVisible = 5 }: WorkflowCollaboratorAvatarsProps) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`workflow-collaborators:${workflowId}`)
      .on(
        'postgres_changes',
        {
          event: '*',
  ...
  const loadCollaborators = async () => {
    try {
      ...
      if (workflow) {
        const profile = workflow.profiles as any;
        setOwner({
          id: workflow.user_id,
