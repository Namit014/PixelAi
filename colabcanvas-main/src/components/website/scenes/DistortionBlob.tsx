...
/** Slow-morphing distorted sphere — premium product / SaaS feel. */
export function DistortionBlob({ primary, accent }: { primary: string; accent: string }) {
    ...
    <mesh ref={meshRef} position={[0, 0, 0]} scale={1.4}>
      ...
      <shaderMaterial
        ...
        vertexShader={`
          ...
          void main(){
            vNormal = normal;
            float n = snoise(position * 1.5 + vec3(uTime * 0.25));
        ...
        fragmentShader={`
          ...
          void main(){
            ...
            float t = smoothstep(-1.0, 1.0, vPos.y);
            vec3 base = mix(uColorA, uColorB, t);
            vec3 col = mix(base, vec3(1.0), fres * 0.6);
